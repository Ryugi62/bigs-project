...
export async function POST(_req: NextRequest) {
  try {
    ...
    if (!refreshToken) return jsonError(401, 'No refresh token', undefined, 'UNAUTHORIZED');
    ...
    if (e instanceof HttpError) return jsonError(e.status, e.message, e.data, 'REFRESH_FAILED');
    return jsonError(500, 'Refresh failed', undefined, 'REFRESH_FAILED');
  }
}
